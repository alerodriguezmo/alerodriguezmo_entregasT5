...
int main(void){
	...
	while(1){
		if(printMsg > 4){
			writeMessage(&usart2Comm, "Hola mundo!");
			printMsg = 0;
		}
	}
}
...
void init_System(void){
	...
	handlerBlinkyTimer.TIMx_Config.TIMx_speed				= 80000; // Pre escalado correspondiente a 1ms para un CLK de 80MHz = 80000
	...
	usart2Comm.USART_Config.USART_freq		   = 80;

	// Cargamos la configuración
	USART_Config(&usart2Comm);

	// Configuración del PLL
	handlerPLL.PLL_src			= HSI; // Se selecciona el reloj interno de 16MHz como referencia

	// Se aplican los factores necesarios para obtener una nueva señal de reloj de 80MHz
	handlerPLL.PLL_pllm			= 8;
	handlerPLL.PLL_plln			= 80;
	handlerPLL.PLL_pllp			= 2;

	// Se cargan la configuración
