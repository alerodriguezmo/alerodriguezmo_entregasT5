...
int main (void){
	...
	sendSTR_toLCD(&handlerLCD, "V [m/s] = ");

	// Etiquieta de tempo de vuelo
	moveCursor_inLCD(&handlerLCD, 3, 1);
	sendSTR_toLCD(&handlerLCD, "Dir [deg] = ");
	...
	while(1){
		...
		if(counterLCD > 4){
			...
			sprintf(bufferV, "%.2f", V);
			sprintf(bufferDir, "%.2f", direction);
			...
			sendSTR_toLCD(&handlerLCD, bufferV);

			moveCursor_inLCD(&handlerLCD, 3, 13);
			sendSTR_toLCD(&handlerLCD, bufferDir);
...
void measureTOF_X1(void){
	for(int i = 0; i<4; i++){
		...
		timeOfFlightDC = (200*(float)(stopwatch+430)/ 100000000) - 0.00040525; // Factor de corrección experimental
